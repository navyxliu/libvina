...
struct matmul
{
  ...
  typedef ARG0                  Arg0;
  typedef ARG1                  Arg1;
  typedef mappar2< matmul, K, false,
  ...
  const static bool _pred = Pred<T, view_trait2<SubWView>::WRITER_SIZE_X,
				 view_trait2<SubWView>::WRITER_SIZE_Y>::value;


  typedef matmul<SubWView, SubRView0, SubRView1,
		 Func, Pred, K> SubTask;
...
struct p_simple{
  const static int value = size_x <= MM_TEST_GRANULARITY;
};
...
{  
  ...
  typedef matmul<Writer, TestMatrix, TestMatrix, 
   matMAddWrapper, p_simple> TF; // fransformer :~)
  ...
  typedef matmul<Writer_v, TestMatrix_v, TestMatrix_v,
    matMAddWrapper, p_simple> TF_SSE;
