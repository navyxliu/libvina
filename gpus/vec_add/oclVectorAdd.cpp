...
int main(int argc, char **argv)
{
    ...
    shrLog(LOGBOTH, 0.0, "clCreateKernel (VectorAdd)...\n"); 
    if (ciErr1 != CL_SUCCESS)
    {
    ...
    if (ciErr1 != CL_SUCCESS)
    {
        shrLog(LOGBOTH, 0.0, "Error in clEnqueueNDRangeKernel, Line %u in file %s !!!\n\n", __LINE__, __FILE__);
        Cleanup(EXIT_FAILURE);
    }

    // Synchronous/blocking read of results, and check accumulated errors
    ciErr1 = clEnqueueReadBuffer(cqCommandQue, cmDevDst, CL_TRUE, 0, sizeof(cl_float) * szGlobalWorkSize, dst, 0, NULL, NULL);
    shrLog(LOGBOTH, 0.0, "clEnqueueReadBuffer (Dst)...\n\n"); 
    if (ciErr1 != CL_SUCCESS)
    {
        shrLog(LOGBOTH, 0.0, "Error in clEnqueueReadBuffer, Line %u in file %s !!!\n\n", __LINE__, __FILE__);
        Cleanup(EXIT_FAILURE);
    }
    //--------------------------------------------------------
    ...
    shrLog(LOGBOTH, 0.0, "TEST %s\t (Error Count = %i)\n\n ", (iNumErrors == 0) ? "PASSED" : "FAILED !!!", iNumErrors);
