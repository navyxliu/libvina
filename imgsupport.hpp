...
namespace vina{
  ...
  public:
	...
	channel = color_type_ & PNG_COLOR_MASK_ALPHA ?
	  (channel+1) : channel;
      }
      return channel;
    }
    ...
    */
    unsigned char * getData() {
      assert( file_ != 0 && reader_ 
	      && "reader file is invalid");

      unsigned char * data = (unsigned char *)malloc
	(height_ * png_get_rowbytes(handle_, info_));
    ...
    void setData(unsigned char * raw) {
      assert( file_ != 0 && !reader_
	      && "writer file is invalid");

      int bytes_per_pixel = getChannel() * (bit_depth_ >> 3);
      unsigned char * row_pointers[height_];
      png_set_IHDR(handle_, info_, width_, height_, bit_depth_,
		   color_type_, 0, 0, 0);
